...
const useChatStore = create((set, get) => ({
    messages: [],
    users: [],
    selectedUser: JSON.parse(Cookies.get("selectedUser") || null),
    isUsersLoading: false,
    isMessagesLoading: false,
    ...
    setSelectedUser: (user) => {
        Cookies.set("selectedUser", JSON.stringify(user), { expires: 7 });
        set({ selectedUser: user });
    },
    ...
    sendMessage: async(messageData) => {
        const { selectedUser, messages } = get();
        try {
            const res = await axiosInstance.post(`/chats/send-web/${selectedUser.id}`, messageData);
            set({ messages: [...messages, res.data] });
        } catch (error) {
            toast.error(error.response.data.message);
        }
    ...
    subscribeToMessages: () => {
        ...
        // Pastikan socket dan selectedUser tersedia
        if (!socket || !selectedUser) {
            console.error("Socket or selectedUser is not available.");
            return;
        }
    ...
    unsubscribeFromMessages: () => {
        const socket = useAuthStore.getState().socket;
        if (socket) {
            socket.off("newMessage"); // Hapus listener
        } else {
            console.error("Socket is not connected. Unable to unsubscribe from messages.");
        }
    },
