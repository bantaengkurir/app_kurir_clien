...
// const OrderTracking = ({ orderId, customerId, latCourier, lnCourier, latCustomer, lnCustomer }) => {
//   const [courierPosition, setCourierPosition] = useState(null);
//   const [customerPosition] = useState([latCustomer, lnCustomer]); // Posisi customer (contoh)
//   const [map, setMap] = useState(null);
//   const [routingControl, setRoutingControl] = useState(null);

//   console.log("latkurir", latCourier)
//   console.log("lnCourier", lnCourier)
...
const OrderTracking = ({ orderId, customerId, latCustomer, lnCustomer }) => {
    const [courierPosition, setCourierPosition] = useState(null);
    const [map, setMap] = useState(null);
    const [routingControl, setRoutingControl] = useState(null);

    // Icon khusus untuk kurir
    ...
    const handleLocationUpdate = (data) => {
        ...
        if (isValidCoordinate(lat, lng)) {
            console.log("Received courier location update:", lat, lng); // Debug log
            setCourierPosition([lat, lng]);
        } else {
            console.error("Invalid coordinates received:", data); // Debug log
        }
    };

    // Update rute saat posisi berubah
    useEffect(() => {
        if (map && courierPosition && latCustomer && lnCustomer) {
            // Hapus rute sebelumnya
            if (routingControl) {
                map.removeControl(routingControl);
            }
            ...
            const newRoutingControl = L.Routing.control({
                waypoints: [
                    ...
                    L.latLng(latCustomer, lnCustomer)
                ],
                lineOptions: {
                    styles: [{ color: 'blue', opacity: 0.7, weight: 5 }]
                },
    ...
    return (
        <div style={{ height: "500px", width: "100%" }}>
            ...
            >
                ...
                {/* Marker Customer */}
                <Marker position={[latCustomer, lnCustomer]}>
                    <Popup>
                        <div>
                            <strong>Lokasi Pelanggan</strong>
                            <br />
                ...
                {/* Marker Kurir dengan update real-time */}
                {courierPosition && (
                    <Marker position={courierPosition} icon={courierIcon}>
                        <Popup>
                            <div>
